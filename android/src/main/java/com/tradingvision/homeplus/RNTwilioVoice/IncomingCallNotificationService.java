...
public class IncomingCallNotificationService extends Service {
    ...
    private Notification buildNotification(String text,
        ...
        NotificationCompat.Action rejectAction = new NotificationCompat.Action.Builder(
                ...
                createActionPendingIntent(context, rejectIntent)).build();
        ...
        NotificationCompat.Action answerAction = new NotificationCompat.Action.Builder(
                ...
                createActionPendingIntent(context, acceptIntent)).build();
    ...
    /*
     * Send the CallInvite to the Activity. Start the activity if it is not running
     * already.
     */
    private void sendCallInviteToActivity(CallInvite callInvite, int notificationId) {
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "sendCallInviteToActivity(). Android SDK: " + Build.VERSION.SDK_INT + " app visible: "
                    + isAppVisible());
        }
        SoundPoolManager.getInstance(this).playRinging();

        // From Android SDK 29 apps are prevented to start an activity from the
        // background
